...
namespace S7evemetry.F1_2020.Structures
{
    public class CarLap : CarLapCommon
    {
        ...
        public static CarLap Read(Span<byte> input)
        {
            var size2020 = 24;
            var start = 8;
            var lap = Read<CarLap>(input, size2020+start);
            lap.Sector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start, 2)); //2
            lap.Sector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 2, 2)); //4 
            lap.BestLapTime = BitConverter.ToSingle(input.Slice(start + 4, 4));//8
            lap.BestLapNum = input[start + 8];//9
            lap.BestLapSector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 9, 2));//11
            lap.BestLapSector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 11, 2));//13
            lap.BestLapSector3TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 13, 2));//15
            lap.BestOverallSector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 15, 2));//17
            lap.BestOverallSector1LapNum = input[start + 17];//18
            lap.BestOverallSector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 18, 2));//20
            lap.BestOverallSector2LapNum = input[start + 20]; //21
            lap.BestOverallSector3TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(start + 21, 2));//23
            lap.BestOverallSector3LapNum = input[start + 23]; //24
            return lap;
        }
    }
}
