...
namespace S7evemetry.F1_2020.Structures
{
    ...
    {
        public static new int Size { get; } = 53;
        ...
        public static CarLap? Read(Span<byte> input)
        {
            ...
            lap.Sector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte, 2)); //2
            lap.Sector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 2, 2)); //4 
            lap.BestLapTime = BitConverter.ToSingle(input.Slice(GapStartByte + 4, 4));//8
            lap.BestLapNum = input[GapStartByte + 8];//9
            lap.BestLapSector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 9, 2));//11
            lap.BestLapSector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 11, 2));//13
            lap.BestLapSector3TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 13, 2));//15
            lap.BestOverallSector1TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 15, 2));//17
            lap.BestOverallSector1LapNum = input[GapStartByte + 17];//18
            lap.BestOverallSector2TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 18, 2));//20
            lap.BestOverallSector2LapNum = input[GapStartByte + 20]; //21
            lap.BestOverallSector3TimeInMs = BinaryPrimitives.ReadUInt16LittleEndian(input.Slice(GapStartByte + 21, 2));//23
            lap.BestOverallSector3LapNum = input[GapStartByte + 23]; //24
            return lap;
        }
    }
}
