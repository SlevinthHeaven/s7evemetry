...
using S7evemetry.F1_2020.Readers;
...
namespace S7evemetry.F1_2020.Listeners
{
    public class F1_2020Listener : UdpStreamListener,
        ...
        IObservable<PacketData<PacketHeader, LobbyInfoData, LobbyInfo>>,
        IObservable<PacketData<PacketHeader, FinalClassificationData, FinalClassification>>
    {
        ...
        private readonly ICollection<IObserver<PacketData<PacketHeader, LobbyInfoData, LobbyInfo>>> _lobbyInfoDataObservers;
        private readonly ICollection<IObserver<PacketData<PacketHeader, FinalClassificationData, FinalClassification>>> _finalClassificationDataObservers;
        ...
        private readonly LobbyInfoDataReader _lobbyInfoDataReader;
        private readonly FinalClassificationDataReader _finalClassificationDataReader;

        public F1_2020Listener(IOptions<UdpListenerSettings> options) : base(options)
        {
            ...
            _lobbyInfoDataObservers = new List<IObserver<PacketData<PacketHeader, LobbyInfoData, LobbyInfo>>>();
            _finalClassificationDataObservers = new List<IObserver<PacketData<PacketHeader, FinalClassificationData, FinalClassification>>>();
            ...
            _lobbyInfoDataReader = new LobbyInfoDataReader();
            _finalClassificationDataReader = new FinalClassificationDataReader();
        }
        ...
        public IDisposable Subscribe(IObserver<PacketData<PacketHeader, LobbyInfoData, LobbyInfo>> observer)
        {
            if (!_lobbyInfoDataObservers.Contains(observer))
            {
                _lobbyInfoDataObservers.Add(observer);
            }
            return new Unsubscriber<PacketData<PacketHeader, LobbyInfoData, LobbyInfo>>(_lobbyInfoDataObservers, observer);
        }

        public IDisposable Subscribe(IObserver<PacketData<PacketHeader, FinalClassificationData, FinalClassification>> observer)
        {
            if (!_finalClassificationDataObservers.Contains(observer))
            {
                _finalClassificationDataObservers.Add(observer);
            }
            return new Unsubscriber<PacketData<PacketHeader, FinalClassificationData, FinalClassification>>(_finalClassificationDataObservers, observer);
        }
            ...
            try
            {
                ...
                switch (packetHeader.PacketId)
                {
                    ...
                    case PacketType.LobbyInfo:
                        {
                            var lobbyInfoDataPacket = _lobbyInfoDataReader.Read(input, packetHeader);
                            if (lobbyInfoDataPacket != null)
                                NotifyData(lobbyInfoDataPacket);
                            break;
                        }
                    case PacketType.FinalClassification:
                        {
                            var finalClassificationDataPacket = _finalClassificationDataReader.Read(input, packetHeader);
                            if (finalClassificationDataPacket != null)
                                NotifyData(finalClassificationDataPacket);
                            break;
                        }
                    default:
        ...
        protected void NotifyData(PacketData<PacketHeader, LobbyInfoData, LobbyInfo> data)
        {
            foreach (var observer in _lobbyInfoDataObservers)
            {
                observer.OnNext(data);
            }
        }

        protected void NotifyData(PacketData<PacketHeader, FinalClassificationData, FinalClassification> data)
        {
            foreach (var observer in _finalClassificationDataObservers)
            {
                observer.OnNext(data);
            }
        }
